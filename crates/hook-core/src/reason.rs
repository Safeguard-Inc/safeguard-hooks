...
///
/// Every variant here is *produced* by a real gate in the enforcement
/// layer — the completeness audit removed reasons no code path could ever
/// emit (admin-caller failures surface as host-level `require_auth` reverts,
/// not contract errors; sanctions/jurisdiction distinctions are invisible
/// on the `is_authorized(account, token) -> bool` policy wire; the contract
/// holds no registration state). Codes are deliberately non-dense so the
/// remaining numbers never shift.
...
pub enum RejectionReason {
    ...
    AccountFrozen,
    /// The spender of a delegated flow is denied by the policy. The spender
    /// holds no funds, so its denial is a distinct, less-privileged outcome
    /// from the fund-holders' denial.
...
impl RejectionReason {
    ...
    /// Stable numeric code. Mirrors `schemas/rejection.schema.json`. Codes
    /// are non-dense: removed reasons are never reissued to a new variant.
    ...
    /// these 2–10, non-dense; used by the operator CLI to decode
    /// `Error(Contract, #N)`).
...
mod tests {
    ...
    fn codes_round_trip_and_all_are_covered() {
        assert_eq!(RejectionReason::ALL.len(), 7);
        for reason in RejectionReason::ALL {
            assert_eq!(RejectionReason::from_code(reason.code()), Some(reason));
        }
        // The reason codes are 2, 3, 4, 5, 8, 9, 10 — deliberately
        // non-dense: 1, 6, 7, 11 were removed by the completeness audit and
        // are never reissued (see docs/errors.md). The contract adds its
        // own codes outside this range (AlreadyInitialized = 12).
        for code in [2, 3, 4, 5, 8, 9, 10] {
            assert!(RejectionReason::from_code(code).is_some());
        }
        for code in [0, 1, 6, 7, 11, 12] {
            assert_eq!(RejectionReason::from_code(code), None);
        }
    }
}
